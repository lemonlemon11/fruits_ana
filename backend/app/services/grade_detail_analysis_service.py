...
# v3-schemeA-3 = 价格文案改为整数「平均每公斤售价」，等级细分数据包同步新口径，旧缓存失效。
# v3-schemeA-2 = 单号序号不再保留字母标记（`宝贝L004` → `宝贝-004`），旧缓存失效。
PROMPT_VERSION = "v3-schemeA-3"
...
4. 平均每公斤售价保留整数（四舍五入），占比写成百分比保留一位小数，不要写 0.4273 这种小数占比；
   件数和金额照抄数据里的原样。
    ...
    return [
        {
            ...
            "平均每公斤售价": row.get("weighted_avg_price"),
...
def build_grade_detail_payload(
    ...
    *,
    ...
    return {
        "口径": (
            "件数单位=件；金额单位=元；平均每公斤售价=销售金额÷销量（千克）（元/公斤）；"
