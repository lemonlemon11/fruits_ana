...
def _price_rankings(buckets: list[dict]) -> list[dict]:
    ...
    rows.sort(key=lambda row: row["平均每件售价"], reverse=True)
    return rows
        ...
        gaps.append(
            {
                "大等级": grade,
                "最贵号别": top["label"],
                "最贵平均每件售价": top["weighted_avg_price"],
                "最便宜号别": bottom["label"],
                "最便宜平均每件售价": bottom["weighted_avg_price"],
...
def _grade_rollup(
    ...
    """把号别桶按大等级 A/B/C 合并好，避免模型自己去加号别的数字。"""
        ...
        rows.append(
            {
                ...
                "件数": rounded(quantity),
                "金额": rounded(amount),
                "平均每件售价": rounded(amount / quantity) if quantity else None,
                "件数占比": quantity_share,
                "金额占比": amount_share,
                "金额占比减件数占比": (
        ...
        rows.append(
            {
                ...
                "带标记平均每件售价": marked_price,
                "无标记件数": plain_metrics["sales_quantity"],
                "无标记平均每件售价": plain_price,
