...
9. 各等级小节各写 2 到 3 条：一条说跨结算单谁高谁低、差多少，
...
def build_cache_key(
    *,
    ...
    current_merchant_no: str | None = None,
) -> str:
    """按功能、结算单范围与日期范围生成缓存键。

    大多数分析不区分“当前结算单”，只关心勾选集合，因此集合会排序后参与哈希；
    结算单详情这类以某张单为主视角的分析，可额外传 ``current_merchant_no``，
    避免同品牌下不同主视角生成同一个缓存键。
    """
    ...
    if current_merchant_no is not None:
        identity["current_merchant_no"] = current_merchant_no.strip()
