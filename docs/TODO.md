...
- [ ] 全系统业务逻辑与交互检查（2026-09-10 用户提出，**进行中**）：
      逐条复核结算单 / 商号 / 单号、到达日期、A/B/C 等级口径、导入覆盖、筛选联动、
      AI 分析结论等主流程的业务正确性与交互一致性，桌面端 + 移动端都要覆盖；
      发现问题先记录现象、触发条件、影响范围与严重程度，再进入修复。
      现有前端 62 项用例 + typecheck 只保证已覆盖路径不回归，不能替代这次人工逻辑复核。

- [ ] 全站字号统一（1rem = 17px，正文不小于 14px）的浏览器端回归验收（**进行中**）：
      覆盖销售总览 / 数据明细 / 结算单对比 / 结算单详情 / 数据导入 / 系列对比 / 登录 / 注册，
      桌面端 + 移动端确认无文字截断、无横向溢出、无固定高度容器被文字撑破。

- [ ] 下拉框统一「默认选中 + 切换立即刷新」的浏览器端手工验收（代码、自动化与提交已完成）：
      确认总览页 / 数据明细 / 结算单详情三处商号下拉切换即刷新，
      到达日期输入仍不自动查询、点「查看结果」才刷新，结算单详情默认选中第一张。
...
> 修复（2026-09-10，代码完成待提交）：「A/B/C 平均每件售价对比」柱状图此前只显示 2px 细线，
...
- [ ] 「系列对比」页与 AI 分析结论的浏览器端验收（自动化用例已覆盖，尚无真实浏览器验收）
