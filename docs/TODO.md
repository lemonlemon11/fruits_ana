...
- [x] **等级细分（2026-09-11 交付，ADR-013）**：从 `grade_raw` 派生号别（方案 A：区间原样成桶），
      ...
      **待验证**：真实浏览器端到端验收（当前用 dev-preview 预览 + 自动化用例覆盖）。

- [ ] 商号规范化：统一 `单637` / `640` 两种写法，并评估存量 4 单是否回填（待评审会决策）

- [ ] 结算单选择器改造：当前「平铺所有结算单勾选」在单量变大后不好用，
      拟改为「已选摘要 + 展开式选择面板（搜索 / 系列折叠 / 草稿选择）」，
      详见 `docs/superpowers/plans/2026-09-11-settlement-picker.md`
