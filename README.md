...
销售金额、平均每千克售价和数量占比。统计等级由 `fruits_ana_admin` 的字段转换规则动态生成，
明细始终保留用户原始写法（如 `BC`）。结算单以商号（我司商业合同唯一单据号）为身份，柜号仅作为车柜标识；
...
- 已打开页面使用顶部页签管理，首页固定不可关闭；右键页签可刷新当前、关闭当前、关闭其他或关闭全部。
  页签只保留当前会话，登录或刷新后从首页重新开始，不缓存上次打开的页签。
- header 铃铛集中展示站内通知，未读消息以角标和侧边小方块提示；点击消息可在详情弹层中查看完整内容。
- 右下角悬浮机器人「顺仔」支持自然语言提问查数据（仅拥有 `ask:view` 权限的角色可见，
  当前为 `fruit_admin`）：机器人气泡在左、自己的提问在右，
  回答中的数字全部取自系统现有数据；系统没有的字段（如采购成本、利润）会直接说明。
  窗口打开时“回顶部”按钮自动避让，按 Esc 或点按钮可收起。
