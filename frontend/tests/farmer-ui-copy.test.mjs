...
test('菜单使用确认后的四个中文入口', () => {
  ...
  for (const label of ['销售总览', '货柜对比', '货柜详情', '数据导入']) assert.match(shell, new RegExp(label))
  for (const icon of ['ChartColumn', 'GitCompareArrows', 'PackageSearch', 'Upload']) assert.match(shell, new RegExp(icon))
  assert.doesNotMatch(shell, /经营总览|单柜详情|导入数据/)
})

test('页面内部不出现跨菜单跳转入口', () => {
  const pageFiles = ['OverviewView.vue', 'ContainerComparisonView.vue', 'ContainerView.vue', 'ImportView.vue']
...
test('货柜列表只展示当前页面的对比结果', () => {
  const comparison = fs.readFileSync(path.join(root, 'components', 'ContainerComparison.vue'), 'utf8')
...
test('货柜详情只使用最近一次成功查询的货柜编号展示结果', () => {
  const detailView = fs.readFileSync(path.join(root, 'views', 'ContainerView.vue'), 'utf8')
  assert.match(detailView, /const activeContainerId = ref\(''\)/)
  assert.match(detailView, /activeContainerId\.value = requestedContainerId/)
  assert.match(detailView, /item\.containerId === activeContainerId\.value/)
  assert.doesNotMatch(detailView, /销售周期：\{\{ filters\.containerId/)
})

test('货柜详情使用同期其他货柜作为价格基线并保留链接日期', () => {
  const detailView = fs.readFileSync(path.join(root, 'views', 'ContainerView.vue'), 'utf8')
  assert.match(detailView, /buildOtherContainerGradeBaseline/)
  assert.match(detailView, /其他货柜/)
...
test('四个页面都提供清晰的两步查看说明', () => {
  for (const view of ['OverviewView.vue', 'ContainerComparisonView.vue', 'ContainerView.vue', 'ImportView.vue']) {
