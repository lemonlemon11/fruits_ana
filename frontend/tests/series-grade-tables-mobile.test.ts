...
test('系列对比移动端表格改为纵向卡片，不再依赖横向滚动', () => {
  ...
  assert.match(mobileBlock, /\.grade-table-card \.table-wrap \{ overflow: visible;\s*\}/)
  assert.match(mobileBlock, /\.grade-table-card table \{[\s\S]*?min-width: 0;[\s\S]*?display: block;\s*\}/)
