...
<template>
  <div class="page-stack import-page">
    ...
    <section class="upload-workbench upload-workbench--compact" aria-labelledby="upload-title">
      ...
      <div class="drop-zone" :class="{ dragging }" role="button" tabindex="0" aria-label="选择或拖放结算单" @click="openFilePicker" @keydown="onDropZoneKeydown" @dragenter.prevent="dragging = true" @dragover.prevent @dragleave.prevent="dragging = false" @drop.prevent="onDrop">
        <input id="settlement-files" ref="fileInput" class="sr-only" type="file" multiple accept=".csv,.xlsx" @click.stop @change="onInput">
        <span class="drop-zone__icon" aria-hidden="true">↑</span><div><strong>拖放结算单到这里</strong><span>或点击选择文件</span></div><span class="secondary-button" aria-hidden="true">选择文件</span>
      </div>
