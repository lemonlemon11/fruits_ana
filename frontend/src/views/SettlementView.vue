...
async function loadOptions() {
  ...
  if (!filters.merchantNo && options.value.length) filters.merchantNo = options.value[0].merchantNo
}
...
<template>
  <div class="settlement-dashboard">
    ...
    <section class="how-to" aria-label="查看方法"><strong>怎么查看</strong><span>第一步：按单号选择结算单和日期。第二步：点击“查看结果”。页面只显示当前结算单的数据。</span></section>
    <form class="filter-bar" @submit.prevent="refresh"><label>单号<select v-model="filters.merchantNo" required><option value="" disabled>选择单号</option><option v-for="item in options" :key="item.merchantNo" :value="item.merchantNo">{{ settlementOptionLabel(item) }}</option></select></label><label>开始日期<input v-model="filters.startDate" type="date"></label><label>结束日期<input v-model="filters.endDate" type="date"></label><button class="primary-button" type="submit" :disabled="loading || !filters.merchantNo">{{ loading ? '正在查询' : '查看结果' }}</button></form>
    ...
    <template v-else>
      <section class="settlement-banner"><div class="settlement-identity"><strong>{{ selectedOption ? settlementOptionLabel(selectedOption) : activeMerchantNo }}</strong><small>商号 {{ activeMerchantNo }} · {{ detail?.containerNo ? `柜号 ${detail.containerNo}` : '未登记柜号' }} · 销售周期：{{ periodLabel }}</small></div></section>
...
.settlement-identity small { color: var(--muted); font-size: .84rem; line-height: 1.5; }
