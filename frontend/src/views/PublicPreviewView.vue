...
function showOrderTooltip(event: MouseEvent, order: PreviewOrder) {
  showTooltip(event, {
    ...
    rows: (['A', 'B', 'C'] as Grade[]).map((grade) => ({
      label: gradeLabels[grade],
      value: `${price(order.grades[grade].price)}/件`,
      color: gradeColors[grade],
    })),
...
function showShareTooltip(event: MouseEvent, row: (typeof gradeTotals.value)[number]) {
  showTooltip(event, {
    ...
    rows: [
      ...
      { label: '平均每件售价', value: price(row.price) },
    ],
...
<template>
  <main class="preview-page">
    ...
    <section class="preview-hero" aria-labelledby="preview-title"><div><p class="eyebrow">宝贝系列 · 3 个业务单</p><h1 id="preview-title">A/B/C 果独立分析</h1><p class="hero-copy">行情走强：A 果越卖越贵，B 果量价齐升，C 果占比抬升。</p></div><div class="hero-meta"><span>到达日期</span><strong>8/27 — 9/05</strong><small>按业务单分别核算</small></div></section>

    <section class="metric-grid" aria-label="宝贝系列关键指标"><article class="metric-card"><span>总件数</span><strong>{{ totalQuantity.toLocaleString() }}</strong><small>3 个业务单合计</small></article><article class="metric-card accent-amount"><span>总销售额</span><strong>{{ money(totalAmount) }}</strong><small>A/B/C 全部等级</small></article><article class="metric-card accent-price"><span>平均每件售价</span><strong>{{ price(totalPrice) }}</strong><small>销售额 ÷ 总件数</small></article><article class="metric-card accent-orders"><span>分析单数</span><strong>{{ orders.length }}</strong><small>按到达日期排序</small></article></section>

    <section class="visual-grid" aria-label="价格趋势与等级占比">
      <article class="preview-card trend-card"><header class="card-heading"><div><p class="eyebrow">价格趋势</p><h2>各单平均每件售价</h2></div><span class="heading-note">单位：元/件</span></header><div class="chart-wrap"><svg class="preview-line-chart" :viewBox="`0 0 ${chart.width} ${chart.height}`" role="img" aria-label="宝贝系列 A、B、C 各单平均每件售价趋势图"><g class="chart-grid" aria-hidden="true"><line v-for="tick in [300, 400, 500]" :key="tick" :x1="chart.left" :x2="chart.width - chart.right" :y1="yPosition(tick)" :y2="yPosition(tick)" /><text v-for="tick in [300, 400, 500]" :key="`label-${tick}`" :x="chart.left - 10" :y="yPosition(tick) + 4" text-anchor="end">{{ tick }}</text></g><polyline v-for="grade in (['A', 'B', 'C'] as Grade[])" :key="grade" class="price-line" :points="linePoints(grade)" :stroke="gradeColors[grade]" /><g v-for="grade in (['A', 'B', 'C'] as Grade[])" :key="`dots-${grade}`"><circle v-for="(order, index) in orders" :key="`${grade}-${order.name}`" :cx="xPosition(index)" :cy="yPosition(order.grades[grade].price)" r="5" :fill="gradeColors[grade]" /><circle v-for="(order, index) in orders" :key="`hit-${grade}-${order.name}`" class="price-hit" :cx="xPosition(index)" :cy="yPosition(order.grades[grade].price)" r="14" @mouseenter="showOrderTooltip($event, order)" @mousemove="moveTooltip" @mouseleave="hideTooltip" /></g><g class="chart-x-labels"><text v-for="(order, index) in orders" :key="order.name" :x="xPosition(index)" :y="chart.height - 14" text-anchor="middle">{{ order.name }}</text></g></svg></div><ChartLegend class="legend-row" :items="previewLegend" /></article>
      <article class="preview-card share-card"><header class="card-heading"><div><p class="eyebrow">数量结构</p><h2>等级件数占比</h2></div><span class="heading-note">占全部件数</span></header><div class="share-layout"><div class="share-ring"><svg class="share-ring-chart" viewBox="0 0 136 136" role="img" aria-label="A、B、C 等级件数占比环形图"><circle class="share-ring-track" cx="68" cy="68" :r="ringRadius" /><circle v-for="segment in ringSegments" :key="segment.grade" class="share-ring-segment" cx="68" cy="68" :r="ringRadius" :stroke="segment.color" :stroke-dasharray="`${segment.dash} ${ringCircumference}`" :stroke-dashoffset="segment.offset" @mouseenter="showShareTooltip($event, segment)" @mousemove="moveTooltip" @mouseleave="hideTooltip" /></svg><div class="ring-center"><strong>{{ totalQuantity.toLocaleString() }}</strong><span>总件数</span></div></div><ul class="share-list"><li v-for="row in gradeTotals" :key="row.grade"><span class="legend-item"><i :style="{ background: gradeColors[row.grade] }" />{{ row.grade }} 果</span><strong>{{ percent(row.share) }}</strong><small>{{ row.quantity.toLocaleString() }} 件</small></li></ul></div><p class="chart-caption">B 果件数最多，是当前系列的走量主力；鼠标放在色环或色条上可查看明细。</p></article>
    </section>
    <ChartTooltip :tooltip="tooltip" />

    <section class="preview-card analysis-card" aria-labelledby="grade-analysis-title"><header class="card-heading"><div><p class="eyebrow">分等级核算</p><h2 id="grade-analysis-title">A/B/C 果独立对比</h2></div><span class="heading-note">点击标签切换等级</span></header><div class="grade-tabs" role="tablist" aria-label="选择水果等级"><button v-for="grade in (['A', 'B', 'C'] as Grade[])" :key="grade" :class="['grade-tab', { active: activeGrade === grade }]" type="button" role="tab" :aria-selected="activeGrade === grade" @click="activeGrade = grade"><span :style="{ background: gradeColors[grade] }" />{{ gradeLabels[grade] }}</button></div><div class="table-wrap"><table><thead><tr><th>单号</th><th>到达日期</th><th>件数</th><th>销售额</th><th>平均每件售价</th><th>占本单销售额</th></tr></thead><tbody><tr v-for="item in activeRows" :key="item.order.name"><th scope="row">{{ item.order.name }}</th><td>{{ item.order.date }}</td><td>{{ item.row.quantity }}</td><td>{{ money(item.row.amount) }}</td><td class="table-price">{{ price(item.row.price) }}</td><td>{{ percent(item.row.amount / item.order.totalAmount) }}</td></tr><tr class="total-row"><th scope="row">品牌合计</th><td>—</td><td>{{ gradeTotals.find((row) => row.grade === activeGrade)?.quantity }}</td><td>{{ money(gradeTotals.find((row) => row.grade === activeGrade)?.amount ?? 0) }}</td><td class="table-price">{{ price(gradeTotals.find((row) => row.grade === activeGrade)?.price ?? 0) }}</td><td>—</td></tr></tbody></table></div></section>

    <section class="preview-card gap-card" aria-labelledby="gap-title"><header class="card-heading"><div><p class="eyebrow">等级价差</p><h2 id="gap-title">A-B、B-C 价差与折价</h2></div><span class="heading-note">均价差额</span></header><div class="table-wrap"><table><thead><tr><th>单号</th><th>A 平均每件售价</th><th>B 平均每件售价</th><th>C 平均每件售价</th><th>A-B 价差</th><th>B-C 价差</th><th>B 比 A 低</th></tr></thead><tbody><tr v-for="row in priceGaps" :key="row.name"><th scope="row">{{ row.name }}</th><td>{{ price(row.grades.A.price) }}</td><td>{{ price(row.grades.B.price) }}</td><td>{{ price(row.grades.C.price) }}</td><td class="gap-value">{{ price(row.ab) }}</td><td class="gap-value">{{ price(row.bc) }}</td><td><span class="discount-pill">{{ percent(row.discount) }}</span></td></tr></tbody></table></div></section>

    <section class="ai-preview" aria-labelledby="ai-title"><div class="ai-icon"><Sparkles :size="20" aria-hidden="true" /></div><div class="ai-content"><div class="ai-heading"><div><p class="eyebrow">AI 分析结论 · 预览</p><h2 id="ai-title">一句话看懂宝贝系列</h2></div><span class="ai-status">示例结论</span></div><p class="ai-summary">A 果越卖越贵、B 果量价齐升、C 果占比抬升，整体行情逐柜走强。</p><div class="ai-points"><p><strong>整体总览</strong>总均价从 {{ price(orders[0].totalAmount / orders[0].totalQuantity) }} 升至 {{ price(orders[2].totalAmount / orders[2].totalQuantity) }}，晚到业务单价格更好。</p><p><strong>A 果</strong>合计 {{ gradeTotals[0].quantity.toLocaleString() }} 件，均价 {{ price(gradeTotals[0].price) }}，是利润核心。</p><p><strong>B 果</strong>合计 {{ gradeTotals[1].quantity.toLocaleString() }} 件，占全部件数 {{ percent(gradeTotals[1].share) }}，是走量主力。</p><p><strong>C 果</strong>合计 {{ gradeTotals[2].quantity.toLocaleString() }} 件，02/03 占比抬升，需关注小箱货比例。</p><p><strong>价差与经营提醒</strong>B-C 价差最高达 {{ price(Math.max(...priceGaps.map((row) => row.bc))) }}，建议持续关注 C 果占比和品质分化。</p></div></div></section>
